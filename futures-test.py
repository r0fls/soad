...
async def sell_my_options(broker, strategy='gay_bear'):
    from tastytrade.instruments import FutureOption
    from database.models import Position
    with broker.db_manager.Session() as session:
        positions = session.query(Position).filter_by(strategy=strategy, broker=broker.broker_name).all()
        for position in positions:
            streamer_symbol = FutureOption.get_future_option(broker.session, position.symbol).streamer_symbol
            price = await broker.get_current_price(streamer_symbol)
            await broker.place_future_option_order(position.symbol, position.quantity, 'sell', strategy='gay_bear')


async def buy_atm_option(broker, symbol='/MNQU4', option_type='P'):
    ...
    futures_options = get_future_option_chain(broker.session, symbol.replace('U4', ''))
    ...
    for date in futures_options.keys():
        ...
        filtered_options = [option for option in options if option.underlying_symbol == '/MNQU4' and option.expiration_date == tomorrow]
        specific_options.extend(filtered_options)
    # Get the ATM put option
    put_options = [option for option in specific_options if option.option_type == option_type]
    put_options.sort(key=lambda x: x.strike_price)
    future = Future.get_future(broker.session, symbol)
    ...
    result = await broker.place_future_option_order(put_option.symbol, 1, 'buy', 'gay_bear')
    return result
...
async def main():
    ...
    brokers, strategies = await initialize_system_components(config)
    #option = await buy_atm_option(brokers['tastytrade'])
    sold = await buy_atm_option(brokers['tastytrade'])
